...
- Main action-label offsets are authored UI coordinates, applied before panel scaling: Store All `(-4,+1)`, Recall `(-2,+1)`, Energy `(-1,+1)`, and Depot `(+1.5,+1)` pixels. Do not move the button art or hitboxes to align their text.
- Base upgrades do not emit a screen ripple. A purchased node gives a compact settle, while newly revealed or newly unlocked child nodes spring-slide outward from their parent along the real branch direction. Upgrade requirements render as item icons inside the authored hotbar-slot frame, with text kept in a separate bounded region.
...
- Command Table action labels lose their individual authored offsets, upgrade branches teleport into place, a ripple returns, or requirement copy draws beneath requirement items. Preserve the tested label offsets, parent-to-child reveal slide, no-ripple rule, and slotted cost layout.
