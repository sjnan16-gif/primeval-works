...
- Every Primeval Works screen uses the shared immediate vanilla-backed UI sound layer for open, close, and click feedback. Base upgrades add their distinct confirmation sound. All gameplay audio is deliberately vanilla-backed too: species families use restrained vanilla vocal/step palettes, and eggs, incubation, processing, turbines, turrets, breeding, berry harvest, and observer detection use matching vanilla cues. Positional mod-triggered audio is explicitly recipient-limited to 5-10 blocks: small interactions use 5-6, machines use 8, and only large dinosaurs or defenses reach 10. A low volume is not a substitute for that radius because vanilla sounds otherwise retain their default range. There is no empty custom sound registry or placeholder `sounds.json`; custom `.ogg` files are optional polish, not a runtime dependency.
...
- Wild eggs are world blocks in small, medium, and large pools. Right-click hatches into ownership and leaves one 1-3 Fossil Fragment stack; larger eggs have progressively better second/third-fragment odds. Silk Touch collects the block for incubation. Incubation is in-world with a timer, not a menu.
...
- A rapid Magic Turret burst uses ordinary hurt invulnerability and only its first pulse deals damage, or a saved burst transfers its remaining pulses to a different target. The damage type bypasses cooldown and the persisted target UUID owns the complete burst.
...
- A turret spends power on or attacks a hostile whose nearest Command Table is another player's base. Dart and Magic targeting must remain inside the connected table's radius and nearest-table cell.
