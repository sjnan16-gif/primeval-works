...
- Endgame weapon is the Ancient Reforged Bayonet, not a bow or spear. Its normal hit only connects when the target collision is 1.2-2.2 blocks from the player's eye and pierces a narrow forward group without striking the owner's nearby companions. Normal Minecraft attack recovery still scales damage, but it does not replace the distance skill check. Its bespoke 16-tick stab flips the reverse grip forward, holds at contact, then retracts through a smooth inspect spin in first and third person. The current 16x16 PNG is temporary presentation art, not the final icon/model.
...
- Removed/not planned: Dino Crafter, Cooker/Cooking Station, Extended Piston, Sticky Extended Piston, Enhanced Rail, Water Wheel, Ancient Chest, Ancient Chest Opener, Super Chest, energy dust. The old `enhanced_rail` registry ID remains only as an invisible save-compatibility alias to vanilla Powered Rail.
...
- The Bayonet inherits a vanilla sword/spear swing, hits point-blank or beyond 2.2 blocks, or its narrow pierce catches an owned companion. Both range validation and damage are server-authoritative; the custom first/third-person curve is presentation only.
...
- Automated checkpoint on 2026-08-24 after the egg, hunger, Bayonet, and rapid-beam pass: wild hatches leave one size-weighted stack of 1-3 Fossil Fragments; all work specialties share one persisted hunger-drain clock; close Spinosaurus targets can still be bitten and periodically receive a small clearing shove; and the Magic Turret delivers four five-damage contacts five ticks apart through a cooldown-bypassing custom damage type. Albino open/blink variants remain independent while authored Pteranodon and Spinosaurus saddle pixels bypass recoloring. The Enhanced Rail is absent from content and progression but old saves migrate its block/item ID to vanilla Powered Rail. The Bayonet enforces its 1.2-2.2-block window on the server, retains normal attack-recovery damage scaling, and uses the shared 16-tick reverse-grip stab/hold/inspect curve in first and third person. Pure tests/full build passed, all 49 required GameTests passed, the integrated client loaded the Bayonet mixins and all ten GeckoLib model/animation sets, and the dedicated server reached `Done`. Food Box panel composition, saddle preservation, close-target shove feel, and the Bayonet hand animation still require the normal human in-world visual pass.
