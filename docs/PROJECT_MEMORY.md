...
- Every Primeval Works screen uses the shared immediate vanilla-backed UI sound layer for open, close, and click feedback. Base upgrades add their distinct confirmation sound. This works before custom `.ogg` assets are authored and must not be gated by the currently empty custom sound bank.
