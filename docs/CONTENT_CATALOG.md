...
| Water Turbine | Working | A 3-by-3, one-block-thick cog whose multiblock cells retain water. Its bottom three cells must be waterlogged, exactly one Energy worker can be assigned, and the cog only rotates during active generation. It generates 1.5 times the upgraded Wind Turbine output, and its item form is the authored cog rather than a substitute cube. |
