...
| Wind Turbine | Working | A 3-by-4 multiblock with two phases. The craftable basic turbine produces 60% output; the Processor upgrades it to full output with a hard-expedition Pteranodon Wing Fragment. Both need open air above the rotor and accept one assigned Energy worker per station. |
| Water Turbine | Working | A 3-by-3, one-block-thick cog whose multiblock cells retain water. Its bottom three cells must be waterlogged, and exactly one Energy worker can be assigned. It generates 1.5 times the upgraded Wind Turbine output. |
