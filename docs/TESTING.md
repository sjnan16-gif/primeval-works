...
2. Configure Connected Quarry and hold left-click on one block; then configure Area Quarry and hold each click on its two opposite corners. Each physical press must issue only one mark. Both modes must open one follower picker over the mark, assign without a false `mark expired` message or a second picker, execute only the bounded order, and clear when finished. Repeat immediately after closing the setup screen and double-click one follower slot; delayed settings sync and duplicate delivery must remain harmless. An assigned Area keeps moving edge dashes plus diagonal lines travelling across all four vertical faces while the Whistle is held. Lumber uses one left-clicked log; Harvest and Collect do not consume attack input.
